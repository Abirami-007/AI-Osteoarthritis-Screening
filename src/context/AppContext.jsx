...
const initialState = {
  ...
  bleDevice: null,       // { name, id } or null
  // Raw sensor samples from BLE recording
  sensorSamples: null,   // Array of { timestamp, LF_Acc_X, ... } or null
  // Recording metadata
  recordingDuration: 0,
};
...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_BLE_CONNECTED':
      return { ...state, bleDevice: action.payload, currentStep: 4 };
    case 'SET_BLE_DISCONNECTED':
      return { ...state, bleDevice: null };
    case 'SET_WALKING_COMPLETE':
      return { ...state, currentStep: 5 };
    case 'SET_SENSOR_SAMPLES':
      return { ...state, sensorSamples: action.payload.samples, recordingDuration: action.payload.duration };
