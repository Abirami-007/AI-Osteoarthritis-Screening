...
export default function PatientReport() {
  ...
  // Access gait analysis (supports both camelCase and snake_case)
  const gaitAnalysis = prediction.gaitAnalysis || {};
  const stepCount = gaitAnalysis.stepCount ?? gaitAnalysis.step_count ?? 0;
  const avgStepTime = gaitAnalysis.avgStepTime ?? gaitAnalysis.avg_step_time ?? 0;
  const gaitSymmetry = gaitAnalysis.gaitSymmetry ?? gaitAnalysis.gait_symmetry ?? 0;
  const strideVariability = gaitAnalysis.strideVariability ?? gaitAnalysis.stride_variability ?? 0;
      ...
      <div id="report-content">
        ...
        <div className="report-header">
          ...
          <p style={{ marginTop: 'var(--space-2)', fontSize: 'var(--font-size-xs)', opacity: 0.75 }}>
            ...
            {prediction.modelVersion && ` • Model: v${prediction.modelVersion}`}
          </p>
        </div>
        ...
        <div className="report-body">
          ...
          <div className="report-section">
            <div className="report-section-title">
              <RiskIcon size={20} />
              Preliminary KOA Risk Screening
            </div>
              ...
              <table className="report-table">
                <tbody>
                  <tr>
                    ...
                    <td>{mlPrediction.class_name}</td>
                  </tr>
                  <tr>
                    <th>KOA Probability</th>
                    <td>{(mlPrediction.KOA_probability * 100).toFixed(1)}%</td>
                  </tr>
                  <tr>
                    <th>Healthy Probability</th>
                    <td>{(mlPrediction.Healthy_probability * 100).toFixed(1)}%</td>
                  </tr>
                  <tr>
                    <th>Decision Threshold</th>
                    <td>{mlPrediction.threshold}</td>
                  </tr>
                </tbody>
              </table>
            )}
          ...
          <div className="report-section">
            ...
            <table className="report-table">
              <tbody>
                <tr>
                  <th>Test Duration</th>
                  <td>{sensorData?.duration || state.recordingDuration || 10}s</td>
                </tr>
                <tr>
                  <th>Total Samples</th>
                  <td>{sensorData?.totalSamples || state.sensorSamples?.length || 0}</td>
                </tr>
                <tr>
                  <th>Step Count</th>
                  <td>{stepCount}</td>
          ...
          <div style={{
            ...
            <strong>⚕️ Medical Disclaimer:</strong> This report is generated by an AI-assisted screening system and is intended for preliminary risk assessment only. It does not constitute a medical diagnosis. Please consult a qualified healthcare professional for a definitive diagnosis and treatment plan. The predictions are based on gait sensor data analysis and questionnaire responses processed by a machine learning model, and should be considered alongside other clinical findings.
          </div>
        </div>
      </div>
    </div>
